...
public class MMMOreGenerator implements IWorldGenerator
{
	...
	public void generateSurface(World world, int chunkX, int chunkZ, Random random)
	{
		...
		for (int i = 0; i < MoreMineralsMod.overworldGen.length; i++)
		{
			...
			{
				int amountPerChunk = MoreMineralsMod.overworldGen[i] / 2;
			...
			{
				int amountPerChunk = MoreMineralsMod.dirtGen[i] / 2;
				...
				for (int j = 0; j < amountPerChunk / 4; j++)
				{
					...
					int randPosY2 = CSRandom.nextInt(random, 48, 80); // Sand
																		// wont
																		// generate
																		// lower
																		// than
			...
			{
				int amountPerChunk = MoreMineralsMod.sandGen[i] / 2;
		...
		for (int i = 0; i < MoreMineralsMod.vanillaGen.length; i++)
		{
			...
			int amountPerChunk = MoreMineralsMod.vanillaGen[i] / 2;
			...
			for (int j = 0; j < amountPerChunk / 4; j++)
			{
				...
				int randPosY2 = CSRandom.nextInt(random, 48, 80); // Sand wont
																	// generate
																	// lower
																	// than
																	// that.
				int randPosZ = chunkZ * 16 + random.nextInt(16);
				if (maxHeight >= 32) // Only common ores generate as dirt and
										// sand ores.
	...
	public void generateNether(World world, int chunkX, int chunkZ, Random random)
	{
		...
		for (int i = 0; i < MoreMineralsMod.vanillaGen.length; i++)
		{
			...
			int amountPerChunk = MoreMineralsMod.vanillaGen[i] / 3;
